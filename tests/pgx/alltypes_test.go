...
import (
	...
	"github.com/go-jet/jet/v2/tests/.gentestdata/pgx/jetdb/test_sample/model"
	. "github.com/go-jet/jet/v2/tests/.gentestdata/pgx/jetdb/test_sample/table"
	"github.com/go-jet/jet/v2/tests/.gentestdata/pgx/jetdb/test_sample/view"
	"github.com/go-jet/jet/v2/tests/testdata/results/common"
)

var AllTypesAllColumns = AllTypes.AllColumns.
	Except(
		IntegerColumn("rowid"), // cockroachDB: exclude rowid column
		// AllTypes.Tsvector,
		// AllTypes.TsvectorPtr,
		// AllTypes.Timez,
		// AllTypes.TimezPtr,
		// AllTypes.MoodPtr,
	)
...
func TestAllTypesSelect(t *testing.T) {
	...
	stmt := AllTypes.SELECT(AllTypesAllColumns).
		LIMIT(2)
...
`)
	...
	dest[0].Timez = *toT(&dest[0].Timez, minus8)
	dest[0].TimezPtr = toT(dest[0].TimezPtr, minus8)
	dest[1].Timez = *toT(&dest[1].Timez, minus8)
	dest[1].TimezPtr = toT(dest[1].TimezPtr, minus8)

	dest[0].Timestampz = *toT(&dest[0].Timestampz, plus1)
	dest[0].TimestampzPtr = toT(dest[0].TimestampzPtr, plus1)
	dest[1].Timestampz = *toT(&dest[1].Timestampz, plus1)
	dest[1].TimestampzPtr = toT(dest[1].TimestampzPtr, plus1)
...
func TestAllTypesInsertModel(t *testing.T) {
	...
	testutils.ExecuteInPgxTxAndRollback(t, ctx, pgxPool, func(tx pgx.Tx) {
		var dest []model.AllTypes
		err := pgxV5.Query(ctx, query, tx, &dest)
		require.NoError(t, err)
...
`, "a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11")
	...
	err := pgxV5.Query(ctx, query, pgxConn, &result)
	require.NoError(t, err)
...
`, byteArrHex, byteArrBin)
	...
	testutils.ExecuteInPgxTxAndRollback(t, ctx, pgxPool, func(tx pgx.Tx) {
		...
		err := pgxV5.Query(ctx, insertStmt, tx, &inserted)
		require.NoError(t, err)
...
`, byteArrBin)
		...
		err = pgxV5.Query(ctx, stmt, tx, &dest)
		require.NoError(t, err)
...
`)
	...
	var dest []model.AllTypes
	err := pgxV5.Query(ctx, mainQuery, pgxConn, &dest)
...
`, int8(11), int8(22), 78, 56, 11, 22, 33, 44, int64(11), int16(22), int64(2))
	...
	err := pgxV5.Query(ctx, query, pgxConn, &dest)
...
`)
		...
		err := pgxV5.Query(ctx, stmtJson, pgxPool, &destSelectJson)
		require.NoError(t, err)
...
`, true, false, true, true, int64(2))
	...
	err := pgxV5.Query(ctx, query, pgxPool, &dest)
...
func TestUInt64Overflow(t *testing.T) {
	...
	_, err := pgxV5.Exec(ctx, stmt, pgxPool)
...
`)
	...
	t.Run("json only", func(t *testing.T) {
		...
		err := pgxV5.Query(ctx, stmtJson, pgxPool, &jsonDest)
		require.NoError(t, err)
	})
}
...
`
		testutils.ExecuteInPgxTxAndRollback(t, ctx, pgxPool, func(tx pgx.Tx) {
			...
			windy.PtoAccrual = ptr.Of(getIntervalVal("3hr"))
			...
			err := pgxV5.Query(ctx, stmt, tx, &windy)
			assert.Nil(t, err)
...
`
		testutils.ExecuteInPgxTxAndRollback(t, ctx, pgxPool, func(tx pgx.Tx) {
			...
			employee.PtoAccrual = ptr.Of(getIntervalVal("5h"))
			...
			testutils.AssertStatementSql(t, stmt, expectedQuery)
			err := pgxV5.Query(ctx, stmt, tx, &employee)
			assert.Nil(t, err)
...
`)
		...
		minus8 := time.FixedZone("UTC", -8*60*60)
		destJson[0].Timez = *toT(&destJson[0].Timez, minus8)
		destJson[1].Timez = *toT(&destJson[1].Timez, minus8)

		destJson[0].Timestampz = *toT(&destJson[0].Timestampz, time.UTC)
		destJson[1].Timestampz = *toT(&destJson[1].Timestampz, time.UTC)

		dest[0].Timestampz = *toT(&dest[0].Timestampz, time.UTC)
		dest[1].Timestampz = *toT(&dest[1].Timestampz, time.UTC)
...
`)
	...
	testutils.ExecuteInPgxTxAndRollback(t, ctx, pgxPool, func(tx pgx.Tx) {
		...
		err := pgxV5.Query(ctx, stmt, tx, &res)
		require.NoError(t, err)
		require.Equal(t, `{"firstName":"John","lastName":"Doe"}`, string(res.JSON))
	})
}

var moodSad = model.Mood_Sad

var intervalVal = getIntervalVal("3 days 04:05:06")
var textMultiDimArray = [][]string{
	{"meeting", "lunch"},
...
var allTypesRow0 = model.AllTypes{
	...
	ByteaPtr:             []byte("bytea"),
	Bytea:                []byte("bytea"),
	TimestampzPtr:        toTimestampTz(testutils.TimestampWithTimeZone("1999-01-08 13:05:06 +0100 CET", 0)),
	Timestampz:           *toTimestampTz(testutils.TimestampWithTimeZone("1999-01-08 13:05:06 +0100 CET", 0)),
	TimestampPtr:         toTimestamp(testutils.TimestampWithoutTimeZone("1999-01-08 04:05:06", 0)),
	Timestamp:            *toTimestamp(testutils.TimestampWithoutTimeZone("1999-01-08 04:05:06", 0)),
	DatePtr:              toDate(testutils.TimestampWithoutTimeZone("1999-01-08 00:00:00", 0)),
	Date:                 *toDate(testutils.TimestampWithoutTimeZone("1999-01-08 00:00:00", 0)),
	TimezPtr:             ptr.Of(fmt.Sprintf("%v", testutils.TimeWithTimeZone("04:05:06 -0800"))),
	Timez:                fmt.Sprintf("%v", testutils.TimeWithTimeZone("04:05:06 -0800")),
	TimePtr:              toTime(testutils.TimeWithoutTimeZone("04:05:06")),
	Time:                 *toTime(testutils.TimeWithoutTimeZone("04:05:06")),
	IntervalPtr:          ptr.Of(intervalVal),
	Interval:             intervalVal,
	BooleanPtr:           ptr.Of(true),
	Boolean:              false,
	PointPtr:             &pgtype.Point{P: pgtype.Vec2{X: 2, Y: 3}, Valid: true},
	BitPtr:               ptr.Of(pgtype.Bits{Bytes: []byte{0b10100000}, Len: 3, Valid: true}),
	Bit:                  pgtype.Bits{Bytes: []byte{0b10100000}, Len: 3, Valid: true},
	BitVaryingPtr:        ptr.Of(pgtype.Bits{Bytes: []byte{0b10111100}, Len: 6, Valid: true}),
	BitVarying:           pgtype.Bits{Bytes: []byte{0b10111100}, Len: 6, Valid: true},
	TsvectorPtr:          ptr.Of("'supernova':1"),
	Tsvector:             "'supernova':1",
	UUIDPtr:              uuid.NullUUID{UUID: uuid.MustParse("a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11"), Valid: true},
	UUID:                 uuid.MustParse("a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11"),
	XMLPtr:               ptr.Of("<Sub>abc</Sub>"),
	XML:                  "<Sub>abc</Sub>",
	JSONPtr:              []byte(`{"a": 1, "b": 3}`),
	JSON:                 []byte(`{"a": 1, "b": 3}`),
	JsonbPtr:             []byte(`{"a": 1, "b": 3}`),
	Jsonb:                []byte(`{"a": 1, "b": 3}`),
	...
	JsonbArray:           [][]byte{[]byte(`{"a": 1, "b": 2}`), []byte(`{"a": 3, "b": 4}`)},
